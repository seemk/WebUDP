...
std::string GenerateSDP(const char* certFingerprint, const char* serverIp,
                        const char* serverPort, const char* ufrag,
                        int32_t ufragLen, const char* pass, int32_t passLen,
                        const ICESdpFields* remote) {
  char sdpBuf[2048];
  ...
  rjs::Writer<rapidjson::StringBuffer> writer(buffer);
  doc.Accept(writer);
